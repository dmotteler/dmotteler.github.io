...
def usage(msg="", error=0):
    ...
    print("""Usage: %s [-e file] [-i file] [-s cal] [-o file] [-c xy] [-m range] [-a] [-b] [-l] [-p] [-r]
   where:
...
def main():
    ...
    try:
        opts, args = getopt.getopt(argv[1:], "c:e:i:s:o:m:ablpr")
    ...
    for o, a in opts:
        if o == "-c":
            if a.startswith("-"):
                ...
                cur, old = list(a)
                if cur not in validcur or old not in validold:
    ...
    if old in ['c', 's', 'v']:
        v_events = tuner_events(None, dotypes, caln=None, outext=ext, fromdate=fromdate, todate=todate)
        if not listonly:
